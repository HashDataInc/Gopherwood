...
//    std::cout << "rec1.cs_item_sk = " << rec1.cs_item_sk << ", rec2.cs_item_sk=" << rec2.cs_item_sk << std::endl;
...
//            std::cout << "fileid = 0" << std::endl;
            return 0;
        case 1:
            if (rec1.cs_item_sk < rec2.cs_item_sk) {
                return -1;
            } else if (rec1.cs_item_sk > rec2.cs_item_sk) {
                return 1;
            }
//            std::cout << "fileid = 1" << std::endl;
            return 0;
        case 2:
            if (strcmp(rec1.cs_item_sk, rec2.cs_item_sk) < 0) {
                return -1;
            } else if (strcmp(rec1.cs_item_sk, rec2.cs_item_sk) > 0) {
                return 1;
            }
//            std::cout << "fileid = 2" << std::endl;
            return 0;
        case 3:
            if ((rec1.cs_item_sk < rec2.cs_item_sk ||
                 (rec1.cs_item_sk == rec2.cs_item_sk && strcmp(rec1.cs_item_sk, rec2.cs_item_sk) < 0))) {
                return -1;
            } else if ((rec1.cs_item_sk > rec2.cs_item_sk ||
                        (rec1.cs_item_sk == rec2.cs_item_sk && strcmp(rec1.cs_item_sk, rec2.cs_item_sk) > 0))) {
                return 1;
            }
//            std::cout << "fileid = 3" << std::endl;
            return 0;
    }
}
...
inline uint hashRecord(char *seed, record_t rec, uint mod, unsigned char field) {
    ...
    switch (field) {
        case 0:
            return hashString(rec.cs_item_sk, mod, s);
        case 1:
            return hashString(rec.cs_item_sk, mod, s);
        case 2:
            return hashString(rec.cs_item_sk, mod, s);
