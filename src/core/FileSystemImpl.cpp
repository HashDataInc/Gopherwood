...
void FileSystemImpl::writeDataFromOSS2Bucket(int64_t ossindex, string fileName) {
    ...
    while (readLength > 0) {
        LOG(
                INFO,
                "FileSystemImpl::writeDataFromOSS2Bucket. readLength=%ld, buf = %s",
                readLength,
                buf);
...
int FileSystemImpl::getOneBlockForWrite(int ossindex, string fileName) {
    ...
    while (!((uint64_t) nextBlockIndex < status->getBlockIdVector().size() && isExist)) {
        ...
        LOG(
                INFO,
                "FileSystemImpl::getOneBlockForWrite, blockIDVector size  = %lu",
                blockIDVector.size());
        ...
        if ((uint64_t) nextBlockIndex < blockIDVector.size()) {
            ...
            LOG(
                    INFO,
                    "FileSystemImpl::getOneBlockForWrite, isExist  = %d,newBlockID=%d",
                    isExist,
                    newBlockID);
        }
    }
    ...
    for (uint64_t i = 0; i < blockIDVector.size(); i++) {
        LOG(
                INFO,
                "FileSystemImpl::getOneBlockForWrite, before blockIDVector[i] = %d,",
                blockIDVector[i]);
    }
    /******************TODO FOR TEST****************************/
    ...
    for (uint64_t i = 0; i < blockIDVector.size(); i++) {
        LOG(
                INFO,
                "FileSystemImpl::getOneBlockForWrite, after blockIDVector[i] = %d,",
                blockIDVector[i]);
    }
...
void FileSystemImpl::rebuildFileStatusFromLog(char *fileName) {
    LOG(
            INFO,
            "FileSystemImpl::rebuildFileStatusFromLog, come in the rebuildFileStatusFromLog file");
...
//            std::vector<int32_t> pingBlockVector;
    ...
    if (blockIDSize > 0) {
        ...
        if (lastBlockID >= 0
                && sharedMemoryManager->checkFileNameAndTypeAndSetKick(lastBlockID, fileName)) {

            LOG(
                    INFO,
                    "FileSystemImpl::rebuildFileStatusFromLog. lastBlockID >= 0 and return true ");
...
//            fileStatus->setPingIDVector(pingBlockVector);
    ...
    LOG(
            INFO,
            "endOffsetOfBucket=%ld,blockIDVector.size=  %lu",
            endOffsetOfBucket,
            blockIDVector.size());
    for (uint64_t i = 0; i < blockIDVector.size(); i++) {
...
void FileSystemImpl::catchUpFileStatusFromLog(char *fileName) {
    ...
    while (readLength == 4) {
        ...
        LOG(
                INFO,
                "FileSystemImpl::catchUpFileStatusFromLog read length = %ld, dataSize size =%d",
                readLength,
                dataSize);
        std::string logRecord;
        char res[dataSize];
    ...
    if ((uint64_t) lastBucketIndex < blockIDVector.size()) {
        LOG(
                INFO,
                "FileSystemImpl::catchUpFileStatusFromLog. the lastBlockIndex=%d, and the lastBlockID=%d",
                lastBucketIndex,
                blockIDVector[lastBucketIndex]);
...
void FileSystemImpl::checkAndAddBlockID(char *fileName, std::vector<int32_t> blockIDVector) {
    ...
    for (uint64_t i = 0; i < blockIDVector.size(); i++) {
        ...
        vector<int32_t>::iterator itElement = find(
    ...
    for (uint64_t i = 0; i < previousVector.size(); i++) {
        LOG(
                INFO,
                "FileSystemImpl::checkAndAddBlockID, previousVector block = %d,",
                previousVector[i]);
    }
    LOG(
            INFO,
            "********************FileSystemImpl::checkAndAddBlockID check is right***********************");

    /******************************TODO FOR PRINT********************/
    LOG(
            INFO,
            "FileSystemImpl::checkAndAddBlockID, block vector size = %lu,",
            status->getBlockIdVector().size());
...
void FileSystemImpl::checkAndAddPingBlockID(char *fileName, std::vector<int32_t> blockIDVector) {
    ...
    for (uint64_t i = 0; i < blockIDVector.size(); i++) {
        ...
        std::vector < int32_t > inactiveBlockVector = status->getLruCache()->put(
                tmpBlockID,
                tmpBlockID);
...
void FileSystemImpl::acquireNewBlock(char *fileName) {
    ...
    if (blockIDVector.size() < MIN_QUOTA_SIZE) {
        ...
        std::unordered_map<int, std::string> remainBlockStatusMap =
                sharedMemoryManager->getBlocksWhichTypeEqual2(remainNeedBlocks);
        unsigned int totalSize = remainBlockStatusMap.size() + blockIDVector.size();

        LOG(
                INFO,
                "FileSystemImpl::acquireNewBlock, acquire %lu blocks which type = '2' ",
                remainBlockStatusMap.size());

        if (totalSize == 0) {
            LOG(
                    LOG_ERROR,
                    "FileSystemImpl::acquireNewBlock, acquire blocks fail, do not acquire any blocks");
        }
        ...
        blockIDVector.insert(
                blockIDVector.end(),
                newRemainBlockVector.begin(),
                newRemainBlockVector.end());
    }
...
std::vector<int32_t> FileSystemImpl::deleteVector(std::vector<int32_t> previousVector,
...
std::vector<int> FileSystemImpl::evictBlock(char *fileName,
    ...
    for (const auto &blockStatus : blockStatusMap) {
        ...
        LOG(
                INFO,
                "FileSystemImpl::evictBlock, fileNameInOSS=%s, fileNameInOSStmp=%s.",
                fileNameInOSS.c_str(),
        ...
        LOG(
                INFO,
                "FileSystemImpl::evictBlock. nowBlockStatus = %s, before blockStatus=%s",
                nowBlockStatus.c_str(),
                blockStatus.second.c_str());
        ...
        if (isEqual != 0) {
            ...
            qsReadWrite->renameObject(
                    (char *) fileNameInOSStmp.c_str(),
                    (char *) fileNameInOSS.c_str());

            //4.3.1 . write the previous file log
            LOG(
                    INFO,
                    "FileSystemImpl::evictBlock, before blockIndex = %d, do really job",
                    blockIndex);
            std::vector < int32_t > previousVector;
            previousVector.push_back(-blockIndex);
            string previousRes = logFormat->serializeLog(
                    previousVector,
                    LogFormat::RecordType::remoteBlock);
            ...
            if ((std::strcmp(previousFileName.c_str(), fileName) == 0)
                ...
                LOG(
                        INFO,
                        "FileSystemImpl::evictBlock. change the last bucket ID, before = %d, after= %d.",
                        tmpBlockID,
                        -(lastBucketIndex + 1));
...
void FileSystemImpl::writeDate2OSS(char *fileNameInOSS, int blockID) {
    ...
    LOG(
            INFO,
            "FileSystemImpl::writeDate2OSS, blockID=%d, fileNameInOSS=%s",
            blockID,
            fileNameInOSS);
...
void FileSystemImpl::closeFile(char *fileName) {
    ...
    while (tmpLength == 4) {
        ...
        LOG(
                INFO,
                "FileSystemImpl::closeFile read tmpLength = %ld, dataSize size =%d",
                tmpLength,
                dataSize);
        std::string logRecord;
        char res[dataSize];
...
char *FileSystemImpl::getFilePath(char *fileName) {
    char *filePathName = static_cast<char *>(malloc(
            strlen(fileName) + strlen(FILE_LOG_PERSISTENCE_PATH) + 1));
...
//            LOG(INFO, "9, LogFormat dataStr size = %d", dataStr.size());
    ...
    LOG(
            INFO,
            "FileSystemImpl::writeFileStatusToLog write size = %d,filePathName = %s",
            length,
            filePathName);
...
//TODO ,JUST FOR TEST
void FileSystemImpl::readDataFromFileAccordingToBlockID(int blockID,
        std::shared_ptr<FileStatus> fileStatus, string suffixName) {
    int SIZE = 128;
    ...
    LOG(
            INFO,
            "1. FileSystemImpl::readDataFromFileAccordingToBlockID. filePath=%s. block vector size = %lu",
            filePath.c_str(),
            blockIDVector.size());
    ...
    if (blockID == fileStatus->getLastBucket()) {
        ...
        if (blockID >= 0) {
            ...
            while (endOfBucketOffset > 0) {
                ...
                int64_t readData = qsReadWrite->qsRead(
                        (char *) ossFileName.c_str(),
                        readBuf,
                        leftData);
                writeCharStrUtil(filePath, readBuf, readData);
                readBuf = new char[SIZE];
        ...
        if (blockID >= 0) {
            ...
            for (int j = 0; j < iter; j++) {
                ...
                LOG(
                        INFO,
                        "7. FileSystemImpl::readDataFromFileAccordingToBlockID. come in if. j=%d, readBuf=%s",
                        j,
                        readBuf);
            ...
            for (int j = 0; j < iter; j++) {
                ...
                LOG(
                        INFO,
                        "8. FileSystemImpl::readDataFromFileAccordingToBlockID. readBuf=%s",
                        readBuf);
                writeCharStrUtil(filePath, readBuf, SIZE);
                readBuf = new char[SIZE];
            }
...
void FileSystemImpl::readTotalRandomDataFromVerifyFile(vector<int32_t> randomIndexVector,
    ...
    LOG(
            INFO,
            "FileSystemImpl::readTotalRandomDataFromVerifyFile fileTobeRead=%s",
            fileTobeRead.c_str());
    ...
    LOG(
            INFO,
            "FileSystemImpl::readTotalRandomDataFromVerifyFile fileTobeWrite=%s",
            fileTobeWrite.c_str());
    ...
    for (uint64_t i = 0; i < randomIndexVector.size(); i++) {
        ...
        if (blockID == fileStatus->getLastBucket()) {
            ...
            for (int j = 0; j < iter; j++) {
                ...
                LOG(
                        INFO,
                        "7. FileSystemImpl::readTotalRandomDataFromVerifyFile. come in if. j=%d, readBuf=%s",
                        j,
                        readBuf);
                writeCharStrUtil(fileTobeWrite, readBuf, readData);
                readBuf = new char[SIZE];
