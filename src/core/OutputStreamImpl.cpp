...
OutputStreamImpl::OutputStreamImpl(std::shared_ptr<FileSystemInter> fs, char *fileName, int flag) {
    ...
    LOG(
            INFO,
            "OutputStreamImpl::OutputStreamImpl. getBlockIdVector().size() = %lu,status->getLastBucket() = %d, status->getEndOffsetOfBucket()=%ld",
    ...
    LOG(INFO, "OutputStreamImpl::OutputStreamImpl. endOfOffset = %ld", endOfOffset);
...
//                THROW(InvalidParameter, "Invalid parameter.");
    ...
    try {
        ...
        setError (current_exception());throw;
    }
}
...
void OutputStreamImpl::writeInternal(char *buf, int64_t size) {
    ...
    LOG(
            INFO,
            "cursorOffset = %ld, cursorIndex=%d,cursorBucketID=%d",
            cursorOffset,
            cursorIndex,
            cursorBucketID);
    LOG(INFO, "data write size = %ld, remainOffsetInBlock=%ld", size, remainOffsetInBlock);
    if (size <= remainOffsetInBlock) {
        ...
        while ((size > remainOffsetTotal)) {
            LOG(
                    INFO,
                    "OutputStreamImpl::writeInternal. 2. remainOffsetTotal before =%ld",
                    remainOffsetTotal);
            ...
            LOG(
                    INFO,
                    "OutputStreamImpl::writeInternal. 2. remainOffsetTotal after =%ld",
                    remainOffsetTotal);
        }
    ...
    LOG(INFO, "cursorOffset=%ld", cursorOffset);
...
int64_t OutputStreamImpl::getRemainLength() {
    ...
    LOG(
            INFO,
            "OutputStreamImpl::getRemainLength. i = %d, status->getBlockIdVector().size=%lu",
            i,
            status->getBlockIdVector().size());
    ...
    LOG(INFO, "OutputStreamImpl::remainLength=%ld", remainLength);
    return remainLength;
...
void OutputStreamImpl::seekInternal(int64_t pos) {
    ...
    LOG(INFO, "OutputStreamImpl::seekInternal, theEOFOffset = %ld", theEOFOffset);
...
void OutputStreamImpl::checkStatus(int64_t pos) {
    ...
    LOG(INFO, "OutputStreamImpl::checkStatus pos = %ld", pos);
    if (pos < 0) {
        LOG(LOG_ERROR, "OutputStreamImpl::checkStatus pos can not be smaller than zero");
    }
    //1. check the size of the file
    int64_t theEOFOffset = this->filesystem->getTheEOFOffset(this->fileName.data());
    LOG(INFO, "OutputStreamImpl::checkStatus. theEOFOffset=%ld", theEOFOffset);
...
void OutputStreamImpl::seekToNextBlock() {
    ...
    LOG(
            INFO,
            "OutputStreamImpl::seekToNextBlock before. cursorBucketID=%d, last bucket id =%d ",
            cursorBucketID,
            status->getLastBucket());
    ...
    LOG(
            INFO,
            "OutputStreamImpl::seekToNextBlock after. cursorBucketID=%d, last bucket id = %d",
            cursorBucketID,
            status->getLastBucket());
...
void OutputStreamImpl::deleteFileBucket(int64_t pos) {
    LOG(INFO, "OutputStreamImpl::deleteFileBucket pos = %ld", pos);
    if (pos < 0) {
        LOG(LOG_ERROR, "OutputStreamImpl::deleteFileBucket pos can not be smaller than zero");
    }
    //1. check the size of the file
    int64_t theEOFOffset = this->filesystem->getTheEOFOffset(this->fileName.data());
    LOG(INFO, "OutputStreamImpl::deleteFileBucket. theEOFOffset=%ld", theEOFOffset);
    ...
    if (pos > theEOFOffset) {
        ...
        LOG(
                LOG_ERROR,
                "OutputStreamImpl::deleteFileBucket. error, the given pos exceed the size of the file");
        return;
    }
