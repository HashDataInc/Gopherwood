 ...
 * If this function return true, GopherwoodCanceled will be thrown.
 */
extern function<bool(void)> ChecnOperationCanceledCallback;

class GopherwoodException;
...
#define THROW(throwable, fmt, ...) \
    Gopherwood::Internal::ThrowException<throwable>(false, __FILE__, __LINE__, #throwable, fmt, ##__VA_ARGS__);
