...
//}
        ...
        Logger::Logger() {
            ...
            fd = open(LOG_FILE_PATH, flags, 0644);
        }
        ...
        void Logger::printf(LogSeverity s, const char *fmt, ...) {
            ...
            try {
                ...
                size = snprintf(&buffer[0], buffer.size(), "%04d-%02d-%02d %02d:%02d:%02d.%06ld, %s, %s ",
                                tm_time.tm_year + 1900,
                                1 + tm_time.tm_mon, tm_time.tm_mday, tm_time.tm_hour,
                                tm_time.tm_min, tm_time.tm_sec, static_cast<long>(tval.tv_usec), ProcessId,
                                SeverityName[s]);
                va_start(ap, fmt);
