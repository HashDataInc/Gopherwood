...
int32_t TcpSocketImpl::read(char * buffer, int32_t size) {
    ...
    do {
        rc = GopherwoodSystem::recv(sock, buffer, size, 0);
...
int32_t TcpSocketImpl::write(const char * buffer, int32_t size) {
    ...
    do {
        ...
        rc = GopherwoodSystem::send(sock, buffer, size, MSG_NOSIGNAL);
#else
        rc = GopherwoodSystem::send(sock, buffer, size, 0);
#endif
    } while (-1 == rc && EINTR == errno && !CheckOperationCanceled());
...
void TcpSocketImpl::connect(const char * host, const char * port, int timeout) {
    ...
    try {
        for (paddr = addrs; NULL != paddr; paddr = paddr->ai_next) {
            ...
            } catch (GopherwoodNetworkConnectException & e) {
                if (NULL == paddr->ai_next) {
                    throw;
                }
            } catch (GopherwoodTimeoutException & e) {
                if (NULL == paddr->ai_next) {
                    throw;
                }
            }
            ...
            if (-1 != sock) {
                GopherwoodSystem::freeaddrinfo(addrs);
                return;
            }
...
void TcpSocketImpl::connect(struct addrinfo * paddr, const char * host, const char * port,
    ...
    try {
        ...
        do {
            rc = GopherwoodSystem::connect(sock, paddr->ai_addr, paddr->ai_addrlen);
        ...
        if (rc < 0) {
            ...
            if (GopherwoodSystem::getpeername(sock, &peer, &len)) {
                /*
                 * connect failed, find out the error info.
                 */
                char c;
                rc = GopherwoodSystem::recv(sock, &c, 1, 0);
                assert(rc < 0);
...
bool TcpSocketImpl::poll(bool read, bool write, int timeout) {
    ...
    do {
        ...
        rc = GopherwoodSystem::poll(&pfd, 1, timeout);
    } while (-1 == rc && EINTR == errno && !CheckOperationCanceled());
...
void TcpSocketImpl::close() {
    if (-1 != sock) {
        GopherwoodSystem::shutdown(sock, SHUT_RDWR);
        GopherwoodSystem::close(sock);
        sock = -1;
    }
}
